...
export enum ENDPOINTS {
    KIVITEL = "Kivitel",
    KIVITEL_FORM = "KivitelForm",
    REMOVE_KIVITEL = "RemoveKivitel"
}
...
export class ClassKivitel extends ClassUtil {
        ...
        this.dataBound(grid, () => {
            ...
            this.gridButtonList(grid, "remove-btn").forEach(item => {
                ...
                item.button.onclick = async () => {
                    ...
                    const response = await this.fetchData(this.url(ENDPOINTS.REMOVE_KIVITEL), {id: dataItem.id});
    ...
    async form(data = {}) {
        const url = this.url(ENDPOINTS.KIVITEL_FORM);
        ...
        return new Promise((resolve) => {
            saveBtn.onclick = async () => {
                ...
                const response = await this.fetchForm(url, formData, popup.form, "kivitel") as ApiResponse;
                resolve(response)
                await popup.close();
            }
        });
    ...
    grid(element: HTMLDivElement): Promise<kendo.ui.Grid> {
        ...
        return new Promise((resolve) => {
            const url = this.url(ENDPOINTS.KIVITEL);
