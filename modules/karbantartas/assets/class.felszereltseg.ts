...
export enum ENDPOINTS {
    ...
    FELSZERELTSEG_FORM = "FelszereltsegForm",
    REMOVE_FELSZERELTSEG = "RemoveFelszereltseg"
}
...
export class ClassFelszereltseg extends ClassUtil {
        ...
        this.dataBound(grid, () => {
            ...
            this.gridButtonList(grid, "remove-btn").forEach(item => {
                ...
                item.button.onclick = async () => {
                    ...
                    const response = await this.fetchData(this.url(ENDPOINTS.REMOVE_FELSZERELTSEG), {id: dataItem.id});
    ...
    async form(data = {}) {
        const url = this.url(ENDPOINTS.FELSZERELTSEG_FORM);
        ...
        return new Promise((resolve) => {
            saveBtn.onclick = async () => {
                ...
                const response = await this.fetchForm(url, formData, popup.form, "felszereltseg") as ApiResponse;
                resolve(response)
                await popup.close();
            }
        });
    ...
    grid(element: HTMLDivElement): Promise<kendo.ui.Grid> {
        ...
        return new Promise((resolve) => {
            const url = this.url(ENDPOINTS.FELSZERELTSEG);
